...
int main(int argc, char** argv)
{
    ...
    for (auto i=3000; i<3641; i++) {
        filenames.push_back("/root/images/" + std::to_string(i+1) + ".jpg");
    }
...
/*
   ...
   // auto format = egt::detail::enum_from_string<egt::PixelFormat>("yuyv");
    auto dev("/dev/video0");
	egt::CameraWindow player(size,dev, egt::PixelFormat::yuyv, egt::WindowHint::software);
	 player.start();
	window.add(player);
*/
 /* Setting camera */
...
/* 	
	...
	timer1.on_timeout([ &cam_fd, &cam_buffer_index,&buffinfo, &heoWin, &frames_camera]() {
       ...
       if (r == -1) {
		...
		//char* yuv_ptr1 = (char*)heoWin.GetOverlay()->raw();
		// djpeg_yuv((char *) cam_buffer_index, sizeof(buffinfo), yuv_ptr1, &width, &height);
...
*/
/*
	auto m_camera = std::make_shared<egt::CameraWindow>(egt::Size(320, 240));
    window.add(m_camera);
    m_camera->on_event([&](egt::Event& event)
    {
		//if(event.id() == egt::EventId::pointer_click)
		//{
            m_camera->move(egt::Point(0, 0));
		...
		for (int i = 0; i < 360; i++)
		{
			for (int j = 0; j <400; j+=2)
			{
				...
				{	
					...
					yuv_ptr[idx+0] = (1.164*(Y0-16.0)+1.596*(V-128.0));
					yuv_ptr[idx+1] = (1.164*(Y0-16.0)-0.813*(V-128.0)-0.391*(U-128.0));
					yuv_ptr[idx+2] = (1.164*(Y0-16.0)+2.018*(U-128.0));
					yuv_ptr[idx+3] = 192;
					//yuv_ptr[idx+3] = 255;
					/*yuv_ptr[idx+4] = (1.164*(Y1-16.0)+1.596*(V-128.0));
					yuv_ptr[idx+5] = (1.164*(Y1-16.0)-0.813*(V-128.0)-0.391*(U-128.0));
