...
*/
   ...
   //middle_text->text_align(egt::AlignFlag::center);
	...
	VDC->box(egt::Rect(middle_box.x()+10.0, middle_box.y()+252.0,10,10));
    VDC->color(egt::Palette::ColorId::label_text, egt::Palette::white);
    VDC->text("Battery");
	VDC->font(egt::Font(14, egt::Font::Weight::bold));
    gauge.add(VDC);

	egt::PeriodicTimer time_timer4(std::chrono::milliseconds(10));
	time_timer4.on_timeout([&VDC]()
    {
     std::string str1 = std::to_string(static_cast<float>(motoValue.VDC));
		std::stringstream ss;
		ss << std::setprecision(3) << motoValue.VDC;
		str1 = ss.str();
		VDC->text("VDC :"+str1+ "mA");
    });
    time_timer4.start();
	//val.Iq;
	auto Iq = std::make_shared<egt::Label>();
 
	Iq->box(egt::Rect(middle_box.x()+160.0, middle_box.y()+252.0,10,10));
    Iq->color(egt::Palette::ColorId::label_text, egt::Palette::white);
    Iq->text("Battery");
	Iq->font(egt::Font(14, egt::Font::Weight::bold));
    gauge.add(Iq);

	egt::PeriodicTimer time_timer5(std::chrono::milliseconds(10));
	time_timer5.on_timeout([&Iq]()
    {
     std::string str1 = std::to_string(static_cast<float>(motoValue.Iq));
		std::stringstream ss;
		ss << std::setprecision(3) << motoValue.Iq;
		str1 = ss.str();
		Iq->text("Iq :"+str1+ "mA");
    });
    time_timer5.start();
	...
	wheel_pulses->box(egt::Rect(middle_box.x()+310.0, middle_box.y()+252.0,10,10));
    wheel_pulses->color(egt::Palette::ColorId::label_text, egt::Palette::white);
    wheel_pulses->text("Battery");
	wheel_pulses->font(egt::Font(14, egt::Font::Weight::bold));
    gauge.add(wheel_pulses);

	egt::PeriodicTimer time_timer9(std::chrono::milliseconds(10));
	time_timer9.on_timeout([&wheel_pulses]()
    {
     std::string str1 = std::to_string(static_cast<float>(motoValue.wheel_pulses));
		std::stringstream ss;
		ss << std::setprecision(3) << motoValue.wheel_pulses;
		str1 = ss.str();
		wheel_pulses->text("wheel_pulses :"+str1+ "p/sec");
    });
    time_timer9.start();
